//! `ST_Y` implementation using `GEOS` via `GeoArrow` arrays
//!
//! This function extracts the Y coordinate of a Point geometry.
...
/// Create the `ST_Y` User Defined Function
///
...
/// SELECT ST_Y(ST_Point(10, 20));
/// -- Returns: 20.0
///
/// -- Extract Y from geometry column
/// SELECT ST_Y(location) FROM cities;
///
/// -- Filter by latitude
/// SELECT * FROM points WHERE ST_Y(geom) > 37.0;
/// ```
///
/// # Arguments
///
...
/// # Returns
///
/// `Float64`: The Y coordinate (latitude), or `NULL` for non-Point geometries
#[must_use]
pub fn create_st_y_udf() -> ScalarUDF {
    ScalarUDF::new_from_impl(StYUDF::new())
}
...
impl datafusion::logical_expr::ScalarUDFImpl for StYUDF {
    ...
    fn invoke_with_args(
            ...
            )
        {
            return Err(DataFusionError::Execution(
                "ST_Y requires GeoArrow geometry input. Use ST_Point, ST_GeomFromText, or ST_GeomFromWKB.".to_string(),
            ));
        }

        let results = compute_y(&geom_array, geo_type, field)
            .map_err(|e| DataFusionError::Execution(format!("ST_Y failed: {e}")))?;

        Ok(ColumnarValue::Array(Arc::new(results)))
    }
}

/// Compute Y coordinates for a geometry array using `GEOS`
fn compute_y(
    arr: &ArrayRef,
    geo_type: Option<&str>,
    ...
    if geo_type == Some(GEOARROW_POINT) || matches!(arr.data_type(), DataType::FixedSizeList(_, 2))
    {
        ...
        for i in 0..len {
            if arr.is_null(i) {
                ...
                builder.append_value(values.value(i * 2 + 1));
            }
        }
    ...
    for i in 0..len {
        ...
        // Only return Y for Point geometries
        ...
        let y = coord_seq
            .get_y(0)
            .map_err(|e| format!("Failed to get Y coordinate at row {i}: {e}"))?;
...
mod tests {
    ...
    #[test]
    fn test_st_y_udf_creation() {
        let udf = create_st_y_udf();
        assert_eq!(udf.name(), "st_y");
    }

    #[test]
    fn test_y_native_points() {
        let points = create_point_array(&[(10.0, 20.0), (-122.4, 37.8), (0.0, 0.0)]);

        let result = compute_y(&points, Some(GEOARROW_POINT), None).unwrap();

        assert_eq!(result.len(), 3);
        assert!((result.value(0) - 20.0).abs() < 1e-10);
        assert!((result.value(1) - 37.8).abs() < 1e-10);
    ...
    fn test_y_wkb_point() {
        ...
        let result = compute_y(&arr, Some(GEOARROW_WKB), None).unwrap();

        assert_eq!(result.len(), 1);
        assert!((result.value(0) - 17.3).abs() < 1e-10);
    }

    #[test]
    fn test_y_non_point_returns_null() {
    ...
    #[test]
    fn test_y_polygon_returns_null() {
    ...
    fn test_y_null_handling() {
        ...
        assert!((result.value(2) - 20.0).abs() < 1e-10);
    }
}
